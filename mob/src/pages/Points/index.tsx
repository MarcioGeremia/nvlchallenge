...
const Points = () => {
  ...
  const handleSelectItem = (id: number) => {
    const alreadySelected = findIndex(selectedItens, (item) => item === id);
    if (alreadySelected >= 0) {
      const filteredItems = filter(selectedItens, (item) => item !== id);
        ...
        >
          {map(items, (item) => (
            <TouchableOpacity
              ...
              style={[
                styles.item,
                includes(selectedItens, item.id) ? styles.selectedItem : {},
              ]}
              onPress={() => {
